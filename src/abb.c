...
void *abb_quitar_recursivo(nodo_abb_t *nodo, void *elemento,
			   abb_comparador comparador, size_t *tamanio)
{
	if (!nodo)
		return NULL;
	...
	if (comparacion < 0) {
		nodo->izquierda = abb_quitar_recursivo(
			nodo->izquierda, elemento, comparador, tamanio);
	} else if (comparacion > 0) {
		nodo->derecha = abb_quitar_recursivo(nodo->derecha, elemento,
						     comparador, tamanio);
	} else {
		if (!nodo->izquierda) {
			...
			nodo->izquierda = abb_quitar_recursivo(
				nodo->izquierda, predecesor->elemento,
				comparador, tamanio);
		}
	}
	return nodo;
...
void *abb_quitar(abb_t *arbol, void *elemento)
{
	...
	if (arbol->comparador(elemento, arbol->nodo_raiz->elemento) == 0) {
		...
		nodo_abb_t *nuevo_raiz = abb_quitar_recursivo(
	...
	nodo_abb_t *nuevo_raiz =
		abb_quitar_recursivo(arbol->nodo_raiz, elemento,
				     arbol->comparador, &(arbol->tamanio));
...
size_t recorrer_preorden_fun(nodo_abb_t *actual,
			     ...
			     size_t *iterados)
{
	...
	bool izquierda = recorrer_preorden_fun(actual->izquierda, funcion, aux,
					       iterados);
	if (!izquierda)
		return false;

	bool derecha =
		recorrer_preorden_fun(actual->derecha, funcion, aux, iterados);
	if (!derecha)
		return false;
...
bool recorrer_inorden_fun(nodo_abb_t *actual, bool (*funcion)(void *, void *),
			  void *aux, size_t *iterados)
{
	if (!actual)
		return true;

	bool izquierda =
		recorrer_inorden_fun(actual->izquierda, funcion, aux, iterados);
	if (!izquierda)
		return false;
	...
	bool derecha =
		recorrer_inorden_fun(actual->derecha, funcion, aux, iterados);
	if (!derecha)
		return false;
...
bool recorrer_postorden_fun(nodo_abb_t *actual, bool (*funcion)(void *, void *),
			    void *aux, size_t *iterados)
{
	if (!actual)
		return true;

	bool izquierda = recorrer_postorden_fun(actual->izquierda, funcion, aux,
						iterados);
	if (!izquierda)
		return false;
	bool derecha =
			     ...
			     bool (*funcion)(void *, void *), void *aux)
{
	...
	if (recorrido == INORDEN) {
		recorrer_inorden_fun(arbol->nodo_raiz, funcion, aux,
				     &funcion_invocada);
	} else if (recorrido == PREORDEN) {
		recorrer_preorden_fun(arbol->nodo_raiz, funcion, aux,
				      &funcion_invocada);
	} else if (recorrido == POSTORDEN) {
		recorrer_postorden_fun(arbol->nodo_raiz, funcion, aux,
				       &funcion_invocada);
	}
...
size_t recorrer_preorden(nodo_abb_t *actual, void **array, size_t tamanio_array,
			 size_t indice)
{
	...
	if (actual->izquierda)
		indice = recorrer_preorden(actual->izquierda, array,
					   tamanio_array, indice);

	if (actual->derecha)
		indice = recorrer_preorden(actual->derecha, array,
					   tamanio_array, indice);

	return indice;
}

size_t recorrer_inorden(nodo_abb_t *actual, void **array, size_t tamanio_array,
			size_t indice)
{
	if (!actual || indice >= tamanio_array)
		return indice;
	...
	if (actual->izquierda)
		indice = recorrer_inorden(actual->izquierda, array,
					  tamanio_array, indice);
	...
	if (actual->derecha)
		indice = recorrer_inorden(actual->derecha, array, tamanio_array,
					  indice);

	return indice;
}

size_t recorrer_postorden(nodo_abb_t *actual, void **array,
			  size_t tamanio_array, size_t indice)
{
	...
	if (actual->izquierda)
		indice = recorrer_postorden(actual->izquierda, array,
					    tamanio_array, indice);

	if (actual->derecha)
		indice = recorrer_postorden(actual->derecha, array,
					    tamanio_array, indice);
