...
void *abb_quitar_recursivo(nodo_abb_t *nodo, void *elemento,
  ...
  if (comparacion < 0) {
    nodo->izquierda =
        abb_quitar_recursivo(nodo->izquierda, elemento, comparador, tamanio);
  } else if (comparacion > 0) {
    nodo->derecha =
        abb_quitar_recursivo(nodo->derecha, elemento, comparador, tamanio);
  } else {
    if (!nodo->izquierda) {
      ...
      nodo->izquierda = abb_quitar_recursivo(
          nodo->izquierda, predecesor->elemento, comparador, tamanio);
    }
  }
  return nodo;
}
...
void *abb_quitar(abb_t *arbol, void *elemento) {
  ...
  if (arbol->comparador(elemento, arbol->nodo_raiz->elemento) == 0) {
    ...
    nodo_abb_t *nuevo_raiz = abb_quitar_recursivo(
        arbol->nodo_raiz, elemento, arbol->comparador, &(arbol->tamanio));
...
size_t recorrer_preorden_fun(nodo_abb_t *actual,
  ...
  bool izquierda =
      recorrer_preorden_fun(actual->izquierda, funcion, aux, iterados);
  if (!izquierda)
    return false;
...
size_t recorrer_postorden(nodo_abb_t *actual, void **array,
  ...
  if (actual->izquierda)
    indice =
        recorrer_postorden(actual->izquierda, array, tamanio_array, indice);
